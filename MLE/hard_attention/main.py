...
from model import HardAttention
from model import REINFORCE_Sampler
...
def train():
    ...
    encoder.cuda()
    generator = HardAttention(config)
    generator.cuda()
    ...
    for e in tqdm(range(config.epoch)):
        ...
        for (images, target, target_mask) in train_dataloader:
            ...
            if config.gradient_estimator == 'REINFORCE':
                REINFORCE_Sampler.loss_logprobs = (torch.gather(-log_probs, 2, target.unsqueeze(2)).squeeze(2) * target_mask).sum(dim=1, keepdim=True) # [batch_size, 1]
...
if __name__ == '__main__':
    if config.mode == 'train':
        train()
        test('hard_attention-' + config.gradient_estimator + '_model/hard_attention-' + config.gradient_estimator)
    elif config.mode == 'test':
        test('hard_attention-' + config.gradient_estimator + '_model/hard_attention-' + config.gradient_estimator)
    elif config.mode == 'decode':
        decode('hard_attention-Gumbel_softmax_model/hard_attention-Gumbel_softmax' + config.gradient_estimator if config.decode_model_path == '' else config.decode_model_path)
