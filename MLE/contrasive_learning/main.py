...
def train():
    target_encoder, target_generator = load_model('reference_model/adaptive_attention')
    reference_encoder, reference_generator = load_model('reference_model/adaptive_attention')
    target_generator.model_name = 'adaptive_attention_CL'
    reference_encoder.eval()
    reference_generator.eval()
    generator_parameters = filter(lambda p: p.requires_grad, target_generator.parameters())
    generator_optimizer = optim.Adam([{'params': generator_parameters}], lr=config.lr, weight_decay=config.weight_decay)

    if not os.path.exists(target_generator.model_name + '_log'):
        os.mkdir(target_generator.model_name + '_log')
    if not os.path.exists(target_generator.model_name + '_models'):
        os.mkdir(target_generator.model_name + '_models')
    if not os.path.exists(target_generator.model_name + '_model'):
        os.mkdir(target_generator.model_name + '_model')
    writer = SummaryWriter(log_dir=target_generator.model_name + '_log', comment=target_generator.model_name + '_train', filename_suffix='.train')
    iteration = 0
    ...
    no_improve = 0
    best_epoch = 0
    print('Training model :', target_generator.model_name)

    for e in tqdm(range(config.epoch)):
        train_data = _CocoCaptions(train_caption_file, 'train', coco_stoi, max_sentence_length, transforms=transforms, flag='CL', K=5)
        train_dataloader = DataLoader(train_data, batch_size=config.batch_size, shuffle=True, num_workers=config.batch_size // 8)
        epoch_sample_num = 0
        epoch_loss = 0
        target_encoder.eval()
        target_encoder.disable_BN()
        target_generator.train()
        current_lr = optimizer_step(generator_optimizer, config, e)
        for (images, target, target_mask, label) in train_dataloader:
            ...
            image_feature, mean_image_feature = reference_encoder(images)
            reference_log_probs = reference_generator(image_feature, mean_image_feature, target, max_step=(int)(target_len.max()))              # [batch_size, max_sentence_length, vocabulary_size]
            image_feature, mean_image_feature = target_encoder(images)
            target_log_probs = target_generator(image_feature, mean_image_feature, target, max_step=(int)(target_len.max()))                    # [batch_size, max_sentence_length, vocabulary_size]
            target_log_prob = (torch.gather(target_log_probs, 2, target.unsqueeze(2)).squeeze(2) * target_mask).sum(dim=1, keepdim=False)       # [batch_size]
            reference_log_prob = (torch.gather(reference_log_probs, 2, target.unsqueeze(2)).squeeze(2) * target_mask).sum(dim=1, keepdim=False) # [batch_size]
            loss = (torch.log(1 + torch.exp(label * (target_log_prob - reference_log_prob)))).mean()

            generator_optimizer.zero_grad()
            loss.backward()
            nn.utils.clip_grad_norm_(target_generator.parameters(), config.gradient_clip)
        ...
        BLEU_1, BLEU_2, BLEU_3, BLEU_4, ROUGE, METEOR, CIDEr = evaluator.evaluate(target_encoder, target_generator, model_info='validate_epoch-' + str(e + 1))
        ...
        torch.save({
            ...
            'generator_state_dict': target_generator.state_dict(),
            'lr': current_lr,
            ...
            'METEOR': METEOR,
            'CIDEr': CIDEr
        }, target_generator.model_name + '_models/' + target_generator.model_name + '-' + str(e + 1))
        if CIDEr > best_CIDEr:
            best_CIDEr = CIDEr
            no_improve = 0
            best_epoch = e + 1
    ...
    shutil.copy(target_generator.model_name + '_models/' + target_generator.model_name + '-' + str(best_epoch), target_generator.model_name + '_model/' + target_generator.model_name)
...
if __name__ == '__main__':
    if config.mode == 'train':
        train()
        test('adaptive_attention_CL_model/adaptive_attention_CL')
    elif config.mode == 'test':
        test('adaptive_attention_CL_model/adaptive_attention_CL')
    elif config.mode == 'decode':
        decode('adaptive_attention_CL_model/adaptive_attention_CL' if config.decode_model_path == '' else config.decode_model_path)
