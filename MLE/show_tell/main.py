...
def train():
    ...
    for e in tqdm(range(config.epoch)):
        ...
        for (images, target, target_mask) in train_dataloader:
            ...
            log_probs = generator(image_feature, mean_image_feature, target, max_step=(int)(target_len.max()))                      # [batch_size, max_sentence_length, vocabulary_size]
...
if __name__ == '__main__':
    if config.mode == 'train':
        train()
        test('show_tell_model/show_tell')
    elif config.mode == 'test':
        test('show_tell_model/show_tell')
    elif config.mode == 'decode':
        decode('show_tell_model/show_tell' if config.decode_model_path == '' else config.decode_model_path)
